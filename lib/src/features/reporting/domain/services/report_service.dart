...
class ReportService {
        ...
        children: [
          ...
          pw.Row(
            ...
            children: [
              _buildInfoColumn(
                'Loan Amount',
                _currency.format(provider.loanAmount ?? 0),
              ),
              _buildInfoColumn(
                'Interest Rate',
              ...
              _buildInfoColumn(
                'Term',
                CurrencyFormatter.formatYears(provider.termYears),
              ),
            ],
          ),
          pw.SizedBox(height: 10),
          pw.Row(
            ...
            children: [
              _buildInfoColumn(
                'Purchase Price',
                _currency.format(provider.price ?? 0),
              ),
              _buildInfoColumn(
                'Down Payment',
                _currency.format(provider.downPayment ?? 0),
              ), // Simplified
      ...
      children: [
        ...
        pw.Table(
          ...
          children: [
            ...
            if (provider.propertyTax != null)
              _buildTableRow('Property Tax', provider.propertyTax! / 12),
            if (provider.homeInsurance != null)
              _buildTableRow('Home Insurance', provider.homeInsurance! / 12),
            if (provider.mortgageInsurance != null)
              _buildTableRow(
                'Mortgage Insurance (PMI)',
                provider.mortgageInsurance! / 12,
              ),
            if (provider.monthlyExpenses != null)
              _buildTableRow('HOA & Other', provider.monthlyExpenses!),
            _buildTableRow(
              'Total Monthly Payment',
              provider.pitiPayment,
              isTotal: true,
            ),
          ],
        ),
  ...
  static pw.Widget _buildAmortizationSummary(
    List<AmortizationEntry> amortization,
  ) {
    if (amortization.isEmpty) return pw.Container();

    final totalInterest = amortization.fold<double>(
      0,
      (sum, item) => sum + item.interest,
    );
    final totalPrincipal = amortization.fold<double>(
      0,
      ...
      children: [
        ...
        pw.Row(
          ...
          children: [
            _buildInfoColumn(
              'Total Principal',
              _currency.format(totalPrincipal),
            ),
