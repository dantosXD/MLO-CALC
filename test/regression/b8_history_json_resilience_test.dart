...
CalculationEntry _validEntry(String id) => CalculationEntry(
  ...
  inputs: const CalculationEntryInputs(loanAmount: 100000, interestRate: 6.5, termYears: 30),
...
void main() {
  group('B8: CalculationHistory.fromJsonString is resilient to corrupt data', () {
    test('one corrupt entry mid-list: valid entries on either side are preserved', () {
      ...
      expect(history.entries.map((e) => e.id), containsAll(['id-before', 'id-after']));
    });

    test('completely invalid JSON leaves existing in-memory entries intact', () {
      final history = CalculationHistory();
    ...
    test('all-valid JSON replaces history atomically', () {
      ...
      final newEntries = [_validEntry('new-1').toJson(), _validEntry('new-2').toJson()];
